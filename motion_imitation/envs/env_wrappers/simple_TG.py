...
class SimpleTG(object):
    ...
    def get_IK(self, tar):
        ...
        tip_2_hip_swing = np.arctan(x , tip_2_hip_extent)

        upper_joint = np.arccos(2 * tip_2_hip_extent * np.cos(tip_2_hip_swing)) - tip_2_hip_swing
        lower_joint = np.pi / 2 + upper_joint - np.arcsin(4 * tip_2_hip_extent - np.cos(upper_joint))

        return np.array([theta , tip_2_hip_extent ,tip_2_hip_swing])
