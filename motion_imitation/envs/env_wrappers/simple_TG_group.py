...
class SimpleTGGroup(object):
    ...
    def __init__(
        ...
        self.action_space = spaces.Box(lg_param_low, lg_param_high, dtype=np.float32)
    ...
    def get_default_upper_bound(self, leg_id):
        ...
        res = np.array([np.pi / 16.0, 0.01, 0.1, 0.1, 0.05, 0.025, 0.1, np.pi, 0.2])
    ...
    """
        ...
        input_param = np.concatenate([np.zeros([1]), input_action]) + self.init_lg_param
